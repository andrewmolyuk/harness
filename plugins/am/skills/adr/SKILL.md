...
- Once `accepted`, an ADR is immutable: you may only fix a typo (spelling, punctuation,
  formatting, a wrong reference — never what was decided), in the session that wrote it,
  resumed or not, and before it's committed. A changed decision is a new ADR that supersedes
  the old one; in the old one, change only its Status and Date.
