# The Guard blocks Leaks into Claude's context; only the user marks an Env file
...
with `am:allow-secret` on its first line is readable, the same rule as `am:no-secrets`; the
Guard denies a Bash command that puts the marker in an Env file, so only the user adds it.
This replaces ADR 0012, whose marker Claude could add itself, loosening the Guard as ADR 0004
forbids.
...
- Letting Claude add the marker, as ADR 0012 did — nothing reviews an uncommitted `.env`, so
  Claude could unlock a file of Secrets and then read it.
...
- Claude can't inspect an unmarked Env file while debugging, nor mark one; the user does both.
- The Edit and Write tools can't add the marker either: both need the file read first.
